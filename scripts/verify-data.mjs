...
// 所有项目（含免费体验伪项目）intro 都不能是空字符串，从源头防止漏文案的数据上线。
const projectsWithoutIntro = PROJECTS.filter((p) => !(typeof p.intro === 'string' && p.intro.trim().length > 0));
assert(projectsWithoutIntro.length === 0, `所有项目都有非空 intro (实际缺失 ${projectsWithoutIntro.length} 条${projectsWithoutIntro.length ? ':\n  ' + projectsWithoutIntro.map((p) => p.id).join('\n  ') : ''})`);
