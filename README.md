...
- 该函数收到 payload 后并行做两件事，互不阻塞（各自 `try/catch`，任一失败不影响另一路，也不影响给前端返回 200）：
  1. 把 payload 转成一条文本消息，POST 到飞书群机器人 webhook；webhook 地址走环境变量 `FEISHU_WEBHOOK_URL`（Vercel 项目设置里配置）
  2. 把 payload 转成 `application/x-www-form-urlencoded` 表单，POST 到合作方的预约测试接口；接口地址通过环境变量 `PREBOOK_API_URL` 配置（Vercel 项目设置里配置，不写进代码和仓库；对方是 http，前端不能直连，只能走这个服务端转发；未配置该环境变量时这一路直接跳过，不影响飞书那一路）
...
| `date` + `time` | 预约日期 + 预约时段 | `book_time`（拼成 `${date} ${time}`） |
