...
- 该函数收到 payload 后并行做两件事，互不阻塞（各自 `try/catch`，任一失败不影响另一路，也不影响给前端返回 200）：
  ...
  2. 把 payload 转成 `application/x-www-form-urlencoded` 表单，POST 到合作方的预约测试接口；接口地址走环境变量 `PREBOOK_API_URL`（代码里有默认值 `<PREBOOK_API_URL>`，对方是 http，前端不能直连，只能走这个服务端转发；对方换正式地址时改环境变量即可，不用改代码）
